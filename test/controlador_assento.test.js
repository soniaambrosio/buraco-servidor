...
const { novoServidor, cliente, T0, METAS_DE_MESA } = require("./ajuda");
...
async function mesa({ humanos = 4, chatPonte = null, gracaAusenciaMs = GRACA, metaPontos = 3000 } = {}) {
  ...
  // [COMPOSICAO meta + controlador] A MESA NASCE CANÔNICA — SEMPRE.
  //
  // Este arnês pedia meta 60 em CTRL-16 para a partida acabar depressa. Depois
  // da meta autoritativa, 60 não é meta de mesa nenhuma e `criarMesa` recusa —
  // corretamente. Não foi aberta porta de teste no servidor para aceitá-la: uma
  // porta dessas faria a suíte inteira medir um servidor que não é o que roda.
  // Quem encurta é o JOGO, depois de iniciado, exatamente como `test/ajuda.js`
  // já fazia. A mesa continua sendo uma mesa que existe em produção.
  const metaDaMesa = METAS_DE_MESA.includes(metaPontos) ? metaPontos : undefined;
  const jogadores = [];
  const dono = await cliente(srv, "uid-0");
  dono.envia({ tipo: "criarMesa", apelido: "Dono", jogadorId: "uid-0", metaPontos: metaDaMesa });
  jogadores.push(dono);
