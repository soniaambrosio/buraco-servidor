...
// [COMPOSIÇÃO] O transporte real agora autentica. O verificador injetado é o de
// PRODUÇÃO (`criarVerificadorFirebase`), só que com certificados de teste —
// assinatura, algoritmo, kid, audience, issuer e validade continuam conferidos.
// Nada é desligado; o que muda é de onde vem o certificado.
const {
  PROTOCOLO_ATUAL,
  emitirToken,
  novoParDeChaves,
...
const PAR = novoParDeChaves("kid-ws");

// RELÓGIO REAL aqui, e não o T0 das outras suítes. Motivo: `iniciar()` monta o
// servidor de produção, cujo relógio de sessão é `Date.now()`. Um token emitido
// em T0 (janeiro) seria aceito pelo verificador e a sessão expiraria no mesmo
// instante, porque `armarExpiracao` compara o `exp` do token com a hora de
// verdade. Emitir agora mantém as duas pontas no mesmo relógio.
const RELOGIO_REAL = { get agoraMs() { return Date.now(); } };
const tokenDe = (uid) => emitirToken({ chave: PAR, uid, emitidoEm: Date.now() });
...
describe("§10 — enforcement no transporte WebSocket real", () => {
  before(() => {
    servidorHttp = iniciar(PORTA, {
      // Verificador de produção com certificados de teste e RELÓGIO REAL — o
      // mesmo do servidor que `iniciar` monta. A expiração da credencial tem
      // suíte própria, com relógio controlado, em `ws_auth.test.js`.
      verificarToken: verificadorDeTeste({ chaves: [PAR], tempo: RELOGIO_REAL }),
    }).http_server;
  ...
  test("pelo fio, o espectador recebe visão pública e o jogador recebe a dele", async () => {
    ...
    for (let i = 0; i < 4; i++) {
      const c = conectar();
      await c.aberto;
      // [COMPOSIÇÃO] Autenticação pela PRIMEIRA mensagem do protocolo, que é o
      // caminho do navegador. O uid do token é o mesmo que o comando declara.
      c.envia({ tipo: "auth", token: tokenDe("ws-" + i), protocolo: PROTOCOLO_ATUAL });
      await c.espera("autenticado");
      jogadores.push(c);
    }
    ...
    // [COMPOSIÇÃO] Ele autentica como `ws-1` — um participante REAL e sentado.
    // É o caso mais duro: identidade legítima, token válido, e ainda assim o
    // assento não o segue. A elevação, se existisse, viria do assento pedido no
    // payload; e é isso que este teste continua provando impossível.
    const esp = conectar();
    await esp.aberto;
    esp.envia({ tipo: "auth", token: tokenDe("ws-1"), protocolo: PROTOCOLO_ATUAL });
    await esp.espera("autenticado");
    esp.envia({
      tipo: "assistirMesa",
      codigo,
